...
NOTA: Temporalmente deshabilitado - instalar flask-socketio para habilitar
"""
...
# Stub temporal para cuando socketio no está disponible
class WebSocketServiceStub:
    ...
    def emit_to_user(user_id, event, data):
        ...
        logger.warning(f"WebSocket deshabilitado - no se pudo enviar evento {event} a usuario {user_id}")
        pass
    
    @staticmethod
    def emit_to_all(event, data):
        """Stub - no hace nada sin socketio"""
        logger.warning(f"WebSocket deshabilitado - no se pudo enviar evento {event} a todos")
        pass

# Usar stub si socketio no está disponible
try:
    from backend.app import socketio
    if not socketio:
        raise ImportError("socketio no inicializado")
    
    # Solo definir el resto si socketio está disponible
