/**
 * Mejoras UI/UX para Coordinador Departamental
...
(function() {
    ...
    function updateStatsCards(data) {
        ...
        if (data.total_municipios !== undefined) {
            document.getElementById('statMunicipios').textContent = data.total_municipios;
        }
        ...
        if (data.total_formularios !== undefined) {
            document.getElementById('statFormularios').textContent = data.total_formularios;
        }
        
        // Actualizar participación
        if (data.participacion !== undefined) {
        ...
        if (data.total_municipios !== undefined) {
            document.getElementById('badgeTodos').textContent = data.total_municipios;
        }
    }

    // ============================================
    // MOBILE CARDS PARA MUNICIPIOS
    // ============================================
    
    function renderMunicipiosMobile(municipios) {
        const container = document.getElementById('municipiosCardsMobile');
        if (!container) return;
        ...
        municipios.forEach(municipio => {
            ...
            }[municipio.estado] || 'dash-circle';
            
            const avance = municipio.avance || 0;
            const progressClass = avance >= 80 ? 'success' : avance >= 50 ? 'warning' : 'danger';
            
            html += `
                <div class="municipio-card" onclick="seleccionarMunicipio(${municipio.id})" data-municipio-id="${municipio.id}">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            <h6 class="mb-1">
                                <i class="bi bi-building"></i> ${municipio.nombre}
                            </h6>
                            <small class="text-muted">${municipio.codigo || ''}</small>
                        </div>
                        <span class="badge bg-${estadoClass}">
                            <i class="bi bi-${estadoIcon}"></i> ${municipio.estado}
                        </span>
                    </div>
                    
                    <div class="mb-2">
                        <small class="text-muted d-block mb-1">Coordinador</small>
                        <strong>${municipio.coordinador_nombre || 'Sin asignar'}</strong>
                    </div>
                    ...
                    <div class="row g-2">
                        <div class="col-4">
                            ...
                            <strong>${municipio.total_puestos || 0}</strong>
                        </div>
                        <div class="col-4">
                            <small class="text-muted d-block">Formularios</small>
                            <strong>${municipio.formularios_enviados || 0}</strong>
                        </div>
                        <div class="col-4">
                            <small class="text-muted d-block">Validados</small>
                            <strong class="text-success">${municipio.formularios_validados || 0}</strong>
                        </div>
                    </div>
                    
                    ${municipio.tiene_discrepancias ? `
    ...
    window.seleccionarMunicipio = function(municipioId) {
        ...
        mostrarDetalleMunicipio(municipioId);
    };
    ...
    function mostrarDetalleMunicipio(municipioId) {
        ...
        if (modal) {
            ...
            // Cargar detalle
            const container = document.getElementById('municipioDetalle');
            if (container) {
                container.innerHTML = `
                ...
                setTimeout(() => {
                    container.innerHTML = `
                        <p class="text-muted text-center">
                            ...
                            Detalle del municipio ${municipioId}
                        </p>
                    `;
                }, 500);
            }
    ...
    function init() {
        console.log('🎨 Inicializando mejoras UI/UX para Coordinador Departamental');
        ...
        window.departamentalMejoras = {
            updateStatsCards,
            renderMunicipiosMobile,
            showToast,
